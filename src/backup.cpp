...
private:
    ...
    ros::NodeHandle nh_;
    message_filters::Subscriber<sensor_msgs::Image> sub_1_, sub_2_, sub_3_, sub_4_, sub_5_;
...
public:
    Backup()
    {
        ...
        sub_2_.subscribe(nh_, "/band/ndvi/image_raw", 1);
        sub_3_.subscribe(nh_, "/band/ndvi_colored/image_raw", 1);
        sub_4_.subscribe(nh_, "/kinect2/hd/image_color_rect", 1);
        sub_5_.subscribe(nh_, "/kinect2/hd/image_depth_rect", 1);
        // Synchronizer policy
        sync_.reset(new Sync(MySyncPolicy(10), sub_1_, sub_2_, sub_3_, sub_4_, sub_5_));
        sync_->registerCallback(boost::bind(&Backup::callback, this, _1, _2, _3, _4, _5));
        // Get package path
    ...
    void callback(const sensor_msgs::ImageConstPtr &in1, const sensor_msgs::ImageConstPtr &in2, const sensor_msgs::ImageConstPtr &in3, const sensor_msgs::ImageConstPtr &in4, const sensor_msgs::ImageConstPtr &in5)
    {
        // Convert from ROS to OpenCV
        cv_bridge::CvImagePtr im_ptr1, im_ptr2, im_ptr3, im_ptr4, im_ptr5;
        try
        {
        ...
        {
            OPERATION = false;
            backupImages(img_1, img_2, img_3, img_4, img_5, path_backup);
        ...
        if (OPERATION)
        {
            backupImages(img_1, img_2, img_3, img_4, img_5, path_stream);
        }
    }

    // Save the backup of the images
    void backupImages(cv::Mat &img_1, cv::Mat &img_2, cv::Mat &img_3, cv::Mat &img_4, cv::Mat &img_5, std::string path)
    {
        // String streams init
        ...
        if (OPERATION)
        {
            ...
            s2 << path << counter << "_multispectral_ndvi.png";
            s3 << path << counter << "_multispectral_ndvi_colored.png";
            s4 << path << counter << "_kinect_hd_rgb.png";
            s5 << path << counter << "_kinect_hd_depth.png";
            counter++;
        }
        else
        {
            s1 << path << 1900 + ltm->tm_year << 1 + ltm->tm_mon << ltm->tm_mday << "_multispectral_camera.png";
            s2 << path << 1900 + ltm->tm_year << 1 + ltm->tm_mon << ltm->tm_mday << "_multispectral_ndvi.png";
            s3 << path << 1900 + ltm->tm_year << 1 + ltm->tm_mon << ltm->tm_mday << "_multispectral_ndvi_colored.png";
            s4 << path << 1900 + ltm->tm_year << 1 + ltm->tm_mon << ltm->tm_mday << "_kinect_hd_rgb.png";
            s5 << path << 1900 + ltm->tm_year << 1 + ltm->tm_mon << ltm->tm_mday << "_kinect_hd_depth.png";
        }
        // Save images
        try
        {
